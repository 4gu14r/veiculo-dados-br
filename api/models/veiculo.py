...
class Modelo(Base):
    ...
    id:        Mapped[int]      = mapped_column(Integer, primary_key=True, autoincrement=True)
    marca_id:  Mapped[int]      = mapped_column(ForeignKey("marcas.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    anos:  Mapped[list["ModeloAno"]]   = relationship(back_populates="modelo", cascade="all, delete-orphan")
...
class ModeloAno(Base):
    ...
    versoes: Mapped[list["Versao"]]  = relationship(back_populates="modelo_ano", cascade="all, delete-orphan")
...
class Versao(Base):
    ...
    id:            Mapped[int]      = mapped_column(Integer, primary_key=True, autoincrement=True)
    modelo_ano_id: Mapped[int]      = mapped_column(ForeignKey("modelos_anos.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    detalhe:    Mapped["VersaoDetalhe"]   = relationship(back_populates="versao", uselist=False, cascade="all, delete-orphan")
...
class VersaoDetalhe(Base):
    ...
    id:        Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    versao_id: Mapped[int] = mapped_column(ForeignKey("versoes.id", ondelete="CASCADE"), nullable=False, unique=True)
